...
open class ImageSlideshowItem: UIScrollView, UIScrollViewDelegate {
    ...
    public let imageViewAnimated = AnimatedImageView()
    ...
    */
    init(image: InputSource, zoomEnabled: Bool, activityIndicator: ActivityIndicatorView? = nil, maximumScale: CGFloat = 2.0) {
        ...
        if image.isAnimatedImage {
            imageViewWrapper.addSubview(imageViewAnimated)
            imageViewAnimated.autoresizingMask = [.flexibleWidth, .flexibleHeight]
            imageViewAnimated.isAccessibilityElement = true
            imageViewAnimated.accessibilityTraits = .image
            if #available(iOS 11.0, *) {
                imageViewAnimated.accessibilityIgnoresInvertColors = true
            }
        } else {
        ...
        if image.isAnimatedImage {
            if UIApplication.shared.userInterfaceLayoutDirection == .rightToLeft {
                imageViewAnimated.transform = CGAffineTransform(rotationAngle: CGFloat(Double.pi))
            }
        } else {
            if UIApplication.shared.userInterfaceLayoutDirection == .rightToLeft {
    ...
    public func loadImage() {
        if image.isAnimatedImage {
            if self.imageViewAnimated.image == nil && !isLoading {
                isLoading = true
                imageReleased = false
                activityIndicator?.show()
                image.loadAnimated?(to: self.imageViewAnimated) {[weak self] image in
                    // set image to nil if there was a release request during the image load
                    if let imageRelease = self?.imageReleased, imageRelease {
                        self?.imageViewAnimated.image = nil
                    } else {
                        self?.imageViewAnimated.image = image
                    }
    ...
    public func cancelPendingLoad() {
        if image.isAnimatedImage {
            image.cancelLoad?(on: imageViewAnimated)
        } else {
    ...
    fileprivate func calculatePictureSize(image: UIImage?, contentMode: UIView.ContentMode) -> CGSize {
    ...
    fileprivate func calculatePictureSize() -> CGSize {
        if image.isAnimatedImage {
            return calculatePictureSize(image: imageViewAnimated.image, contentMode: imageViewAnimated.contentMode)
        } else {
            return calculatePictureSize(image: imageView.image, contentMode: imageView.contentMode)
        }
    }
